...
public class BoardTests {
    ...
    void createEmptyBoard() {
        ...
        assertFalse(board.getStoneAt(Position.at(3,2)).isOfColour(WHITE));
    }
    ...
    void makeStoneWhite() {
        Board board = new Board(5);
        board.updateStoneAt(Position.at(3, 2), WHITE);
        assertTrue(board.getStoneAt(Position.at(3,2)).isOfColour(WHITE));
    }

    @Test
    void adjacentPositionsAllOccupied(){
        Board board = new Board(3);
        board.updateStoneAt(Position.at(1, 2), WHITE);
        board.updateStoneAt(Position.at(2, 1), BLACK);
        board.updateStoneAt(Position.at(2, 2), WHITE);
    ...
    void horizontalLiveStones(){
        Board board = new Board(5);
        board.updateStoneAt(Position.at(1, 1), WHITE);
        board.updateStoneAt(Position.at(2, 1), WHITE);
    ...
    void LiveStones(){
        ...
        for(int x = 1; x <= size; ++x) {
            for(int y = 1; y <= size; ++y) {
                if(x*y % 2 == 0) {
                    board.updateStoneAt(Position.at(x, y), WHITE);
                } else {
                    board.updateStoneAt(Position.at(x, y), BLACK);
                }
            }
        }
        board.checkBoardAndMakeStonesLive();
